...
def show_result():
    ...
    if "final_choice" not in st.session_state:
        ...
        selected = st.radio("마음에 드는 상대를 선택해주세요:", choice_options, index=None)
        
        if st.button("선택 완료 → 분석 결과 보기", type="primary", use_container_width=True, disabled=(selected is None)):
            # 선택 결과 매핑
            choice_map = {
    ...
    if "analysis_result" not in st.session_state:
        if history:
            ...
            st.session_state["analysis_result"] = {"error": "분석할 대화 기록이 없습니다."}
